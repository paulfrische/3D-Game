...
Camera::Camera(glm::vec3 position, float pitch, float yaw, glm::vec3 up, float speed, float mouse_sensitivity, float fov, GLFWwindow* window, unsigned int shader)
{
    ...
    this->projectionMatrix = glm::perspective(glm::radians(fov), 1280.0f / 720.0f, 0.1f, 100.0f);
}

void Camera::Update()
{
    viewMatrix = glm::lookAt(position, position + front, up);
    projectionMatrix = glm::perspective(glm::radians(fov), 1280.0f / 720.0f, 0.1f, 100.0f);
